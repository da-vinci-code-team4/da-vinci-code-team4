...
public class RankingPage extends JPanel {
    ...
    private static List<String[]> rankingData; // ranking.txt에서 읽은 데이터
    ...
    public RankingPage(JPanel mainPanel, CardLayout cardLayout) {
        ...
        rankingData = readRankingData("texts/ranking.txt");
        ...
        JButton backButton = new JButton(
                new ImageIcon(Objects.requireNonNull(getClass().getResource("/img/ViewImage/back.png"))));
    ...
    public static void updateUserRanking(User currentUser, List<User> userList) {
        
        List<String[]> tempList = RankingPage.rankingData;
        ...
        for(String[] data : tempList){
            if(data[0].equals(currentUser.getUsername())){
                Session.getInstance().getCurrentUser().setRanking(tempList.indexOf(data)+1);
                System.out.println("Ranking updated: " + data[0]+" " +(tempList.indexOf(data)+1));
            }
    ...
    private void updateDataPanel() {
        ...
        for (int i = currentPage * ROWS_PER_PAGE; i < Math.min((currentPage + 1) * ROWS_PER_PAGE,
                rankingData.size()); i++) {
            ...
            rankLabel.setFont(new Font("Malgun Gothic", Font.BOLD, 20));
            ...
            ratioLabel.setFont(new Font("Malgun Gothic", Font.BOLD, 20));
    ...
    private JPanel createWinRatePanel(String winsStr, String lossesStr) {
        ...
        winLabel.setFont(new Font("Malgun Gothic", Font.BOLD, 14));
        ...
        lossLabel.setFont(new Font("Malgun Gothic", Font.BOLD, 14));
        lossLabel.setForeground(Color.WHITE);
        lossLabel.setBounds((int) ((double) wins / totalGames * 300), 0, (int) ((double) losses / totalGames * 300),
                30);
        panel.add(lossLabel);
    ...
    public static void setRankingData(List<String[]> rankingData) {
        System.out.println("Ranking data updated"+rankingData.size());
        RankingPage.rankingData = rankingData;
    }
}
